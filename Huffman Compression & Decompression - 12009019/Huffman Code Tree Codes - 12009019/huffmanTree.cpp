...
data huffmanTree::createHuffmanTree(priority_queue<data*, vector<data*>, compare>* huffmanTree)
{
	...
	{
		//set up data node in vector
		_leaf = new data;

		//remove first node from tree
		_leaf->rightChild = huffmanTree->top();
		_leaf->letter = huffmanTree->top()->letter;
		_leaf->frequency =  huffmanTree->top()->frequency;
		huffmanTree->pop();			

		//remove next node in queue
		_leaf->leftChild = huffmanTree->top();
		_leaf->letter += huffmanTree->top()->letter;
		_leaf->frequency +=  huffmanTree->top()->frequency;
		huffmanTree->pop();	

		//add node to vect to be sorted back in queue
		parent->push_back(_leaf);
...
void huffmanTree::compress(map<string, string> &codes, string input)
{
	...
	for (int i = 0; i < currentCode.length(); i +=8)
	{
		...
		for (unsigned bit = 0; bit != 8; ++bit)
		{
			...
			//adds padding however do this before this stage to get pad amount
		/*	else
			{
				byte |= 0 << bit;
			}*/
